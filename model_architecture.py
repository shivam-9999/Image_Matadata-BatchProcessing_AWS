...
def train_model(model, train_loader, val_loader, epochs=30):
    ...
    for epoch in range(epochs):
        ...
        logging.info(f"Epoch [{epoch + 1}/{epochs}], Train Loss: {total_loss / len(train_loader):.4f}, Val Loss: {val_loss:.4f}")
...
if __name__ == "__main__":
    ...
    train_metadata = np.load('processed_train_metadata.npy', allow_pickle=True)
    test_metadata = np.load('processed_test_metadata.npy', allow_pickle=True)
    ...
    for train_file in train_files:
        ...
        logging.info(f"🏁 Evaluation (Haversine Distance): {final_distance:.4f} km")
